...
def fuse_by_igm(full_image, swir_img, vnir_img, kdtree, temp_vnir, coords_swir, rgb_swir, rgb_vnir):
    for i in range(0, swir_img.shape[0], 1):
        ...
        for j in range(0, swir_img.shape[1], 1):
            pixel = [i, j]
            closest_pixel, closest_coords, diff = find_closest_pixel(kdtree, pixel, swir_img, temp_vnir, vnir_img,
                                                                     coords_swir)
            diffs.append(diff)
...
if __name__ == '__main__':
    ...
    path_swir = os.path.join(path_images, "SWIR/3")
    path_vnir = os.path.join(path_images, "VNIR/3")
    flag_igm = False
    ...
    if flag_igm:
        ...
        full_image, rgb_swir, rgb_vnir = fuse_by_igm(full_image, swir_img, vnir_img, kdtree, temp_vnir, coords_swir,
                                                     rgb_swir, rgb_vnir)
        ...
        mean_image, vnir, swir, full_image_shared = sort_fused_image_bands_and_make_hdr(full_image, name_hdr_swir,
                                                                                        name_hdr_vnir, full_swir,
                                                                                        full_vnir, flag_save=False)
        # rgb_mean_image = image_to_rgb (mean_image)
        ...
        mean_image, vnir, swir, full_image_shared = sort_fused_image_bands_and_make_hdr(full_image_new, name_hdr_swir,
                                                                                        name_hdr_vnir, full_swir,
                                                                                        full_vnir)
        rgb_mean_image = image_to_rgb(mean_image)
